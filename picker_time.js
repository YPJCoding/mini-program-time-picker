...
Component({
  ...
  methods: {
    _onInit() {
      ...
      // console.log(year);
      if (this.data.startDate == null && this.data.endDate == null) {
        startDate = this._getDefaultDate("2000-01-01 00:00");
        endDate = this._getDefaultDate("2050-12-31 23:59");
        tpData = this._getModify(date, startDate, endDate);
      }
  ...
  observers: {
    'date': function () {
      if (this.data.first_in) {
        this._onInit();
      }
      // console.log('进入observers节点=', this.data.date)
    }
  }
})